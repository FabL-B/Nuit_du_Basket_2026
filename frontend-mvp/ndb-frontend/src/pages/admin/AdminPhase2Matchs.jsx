...
export default function AdminPhase2Matchs() {
  ...
  const matchsQ = useQuery({
    queryKey: ["admin-matchs", editionId, "PHASE_2"],
    queryFn: () => adminApi.matchs({ edition: editionId, phase: "PHASE_2" }),
    enabled: !!editionId,
  });
  ...
  const genMatchsM = useMutation({
    mutationFn: () => adminApi.genererMatchsPhase(phase2.id),
  ...
  if (!phase2) return <EmptyState label="Phase 2 introuvable. Crée-la d’abord." />;

  return (
    <div className="stack">
      <div className="card">
        <div className="title">Phase 2 — Matchs</div>
