...
def test_match_refuse_si_equipe_pas_bon_tournoi():
    ...
    sous_phase = SousPhase.objects.create(
        phase_globale=phase, tournoi=tournoi_loisir, branche=BrancheSousPhase.AUCUNE
    )
    groupe = Groupe.objects.create(sous_phase=sous_phase, code="A")

    e_loisir = Equipe.objects.create(
        edition=edition, tournoi=tournoi_loisir, nom="E1", statut=StatutEquipe.VALIDEE
    )
    e_rookie = Equipe.objects.create(
        edition=edition, tournoi=tournoi_rookie, nom="E2", statut=StatutEquipe.VALIDEE
    )
