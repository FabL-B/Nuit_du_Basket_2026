...
def test_swap_refuse_si_pas_meme_sous_phase():
    ...
    phase2 = PhaseGlobale.objects.create(
        edition=edition2, type_phase=TypePhaseGlobale.PHASE_1, sequence=1
    )
    sous_phase2 = SousPhase.objects.create(
        phase_globale=phase2, tournoi=tournoi2, branche=BrancheSousPhase.AUCUNE
    )

    g_other = Groupe.objects.create(sous_phase=sous_phase2, code="A")
    e_other = Equipe.objects.create(
        edition=edition2, tournoi=tournoi2, nom="EOTHER", statut=StatutEquipe.VALIDEE
    )
