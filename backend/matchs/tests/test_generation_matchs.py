...
def test_generation_matchs_sur_tous_les_groupes_de_phase_globale():
    ...
    # sous-phases
    sp_r = SousPhase.objects.create(phase_globale=phase, tournoi=t_rookie, branche=BrancheSousPhase.AUCUNE)
    sp_l = SousPhase.objects.create(phase_globale=phase, tournoi=t_loisir, branche=BrancheSousPhase.AUCUNE)
    sp_c = SousPhase.objects.create(phase_globale=phase, tournoi=t_compet, branche=BrancheSousPhase.AUCUNE)
    ...
    def build_group(sp, prefix):
        ...
        equipes = []
        for i in range(4):
            e = Equipe.objects.create(edition=edition, tournoi=sp.tournoi, nom=f"{prefix}{i}", statut=StatutEquipe.VALIDEE)
            equipes.append(e)
    ...
    assert Match.objects.filter(statut=StatutMatch.A_PLANIFIER, creneau__isnull=True, terrain__isnull=True).count() == 18
